...
class CanvasObserver {
     ...
     * 注册监听器
     */
    registerListener(listener: BaseListener | BaseListener[]) {
     ...
     * 移除注册的监听器
     */
    unRegisterListener(listener: BaseListener | BaseListener[]) {
        const listeners = Array.isArray(listener) ? listener : [listener];
        listeners.forEach(i => {
            const index = this.listeners.indexOf(i);
            if (index !== -1) {
     ...
     * @param event 
     */
    private trigger(name: EventName, event: CanvasEvent) {
        this.listeners.forEach(interaction => interaction.trigger(name, event));
    }
    ...
    private onPointerDown = (event: PointerEvent) => {
        ...
        if (isLeft) {
            this.trigger(EventName.LPointerDown, event);
        } else {
            this.trigger(EventName.RPointerDown, event);
        }
    ...
    /**
     * 根据原生 pointermove 触发 Move 交互事件
     */
    private onPointerMove = (event: PointerEvent) => {
        // step 1. 触发 Move 事件
        this.trigger(EventName.Move, event);
        // step 2. 记录在 Canvas 中最新的 Move Event
        this.lastEventInArea = event;
    }

    /**
     * 根据原生 pointerup 触发  LPointerUp/RPointerUp 、Click/RightClick 交互事件
     */
    private onPointerUp = (event: PointerEvent) => {
        ...
        if (this.isPressing) {
            ...
            if (isLeft) {
                this.trigger(EventName.LPointerUp, fixedEvent);

                if (isMoved) {
                    this.trigger(EventName.Click, fixedEvent);
                }
            } else {
                this.trigger(EventName.RPointerUp, fixedEvent);

                if (isMoved) {
                    // TODO 右键的默认行为怎么办？
                    this.trigger(EventName.RightClick, fixedEvent);
                }
            }
        }
    }
    ...
    private onDblClick = (event: PointerEvent) => {
        this.trigger(EventName.DBClick, event);
    }
    ...
    private onWheel = (event: WheelEvent) => {
        this.trigger(EventName.Wheel, event);
    }
